...
/**
 * voice.voicebox_qwen_custom — speak via Voicebox Qwen Custom Voice engine.
 *
 * Custom voice clone via Qwen. ~3.6 GB VRAM.
 * Best for custom cloned voices with Qwen quality.
 */
export default definePlugin({
    id: 'voice.voicebox_qwen_custom',
    name: 'Voicebox Qwen Custom (custom clone)',
    category: 'voice',
    description: 'Generate speech via Voicebox Qwen Custom Voice engine. Custom voice clone. ~3.6 GB VRAM.',
    inputs: {
        ...
        out: S.string('Output file', { default: 'qwen-custom-speech.wav' }),
    },
    outputs: ['audio'],
    async run({ input, ctx }) {
        ...
        const body: Record<string, unknown> = { text, profile, engine: 'qwen_custom_voice' };
        ...
        const dest = ctx.out(String(input.out ?? 'qwen-custom-speech.wav'));
        const buf = Buffer.from(await res.arrayBuffer());
        fs.writeFileSync(dest, buf);
        return { outputs: [{ path: dest, kind: 'audio' as const, meta: { engine: 'qwen_custom_voice', profile, generationId, bytes: buf.length } }] };
    },
});
