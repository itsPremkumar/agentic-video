...
/**
 * voice.voicebox_luxtts — speak via Voicebox LuxTTS engine.
 *
 * Premium-quality TTS engine. VRAM varies by model.
 * Best for highest-fidelity output when quality is paramount.
 */
export default definePlugin({
    id: 'voice.voicebox_luxtts',
    name: 'Voicebox LuxTTS (premium)',
    category: 'voice',
    description: 'Generate speech via Voicebox LuxTTS engine. Premium quality. VRAM varies.',
    inputs: {
        ...
        out: S.string('Output file', { default: 'luxtts-speech.wav' }),
    },
    outputs: ['audio'],
    async run({ input, ctx }) {
        ...
        const body: Record<string, unknown> = { text, profile, engine: 'luxtts' };
        ...
        const dest = ctx.out(String(input.out ?? 'luxtts-speech.wav'));
        const buf = Buffer.from(await res.arrayBuffer());
        fs.writeFileSync(dest, buf);
        return { outputs: [{ path: dest, kind: 'audio' as const, meta: { engine: 'luxtts', profile, generationId, bytes: buf.length } }] };
    },
});
