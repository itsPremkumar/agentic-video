...
/**
 * voice.voicebox_chatterbox — speak via Voicebox Chatterbox engine.
 *
 * Cloned voice, multilingual. ~3.2 GB VRAM.
 * Best for realistic cloned voices with language flexibility.
 */
export default definePlugin({
    id: 'voice.voicebox_chatterbox',
    name: 'Voicebox Chatterbox (cloned, multilingual)',
    category: 'voice',
    description: 'Generate speech via Voicebox Chatterbox engine. Cloned voice, multilingual. ~3.2 GB VRAM.',
    inputs: {
        ...
        out: S.string('Output file', { default: 'chatterbox-speech.wav' }),
    },
    outputs: ['audio'],
    async run({ input, ctx }) {
        ...
        const body: Record<string, unknown> = { text, profile, engine: 'chatterbox' };
        ...
        const dest = ctx.out(String(input.out ?? 'chatterbox-speech.wav'));
        const buf = Buffer.from(await res.arrayBuffer());
        fs.writeFileSync(dest, buf);
        return { outputs: [{ path: dest, kind: 'audio' as const, meta: { engine: 'chatterbox', profile, generationId, bytes: buf.length } }] };
    },
});
